...
// Modified the routine to convert matrices that are not pattern too
...
void coo2csc(uint32_t *const row,           /*!< CSC row start indices */
  ...
  for (uint32_t l = 0; l < nnz; l++) col[col_coo[l] - isOneBased]++;
  ...
  for (uint32_t l = 0; l < nnz; l++) {
    ...
    if (!isPattern) values[dst] = values_coo[l];
